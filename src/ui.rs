...
use crate::{
    types::{Action, CiStatus, Notification, SubjectStatus},
    AppState,
};

const COMMAND_REFERENCE: &str =
    "Commands: o open/read  y yank  r read  d done  u unsub  |  Targets: 1-3, 1 2 3, m merged, c closed, f draft";
    ...
    let items: Vec<ListItem> = app
        ...
        .map(|(idx, notification)| {
            ...
            if widths.ci > 0 {
                ...
                if widths.ci_kind_gap > 0 {
                    header_spans.push(Span::raw(" ".repeat(widths.ci_kind_gap)));
                }
            }
...
fn layout_widths(
    ...
    let mut ci_kind_gap = if ci > 0 { CI_KIND_GAP } else { 0 };
    let mut meta_width = kind + 1 + time + ci + ci_kind_gap;
    ...
    if meta_width + repo_meta_gap > title {
        let max_kind_allowed = title
            .saturating_sub(time + 1 + repo_meta_gap + ci + ci_kind_gap)
            .max(1);
        kind = kind.min(max_kind_allowed);
        meta_width = kind + 1 + time + ci + ci_kind_gap;
    }
    if meta_width + repo_meta_gap > title {
        let max_time_allowed = title
            .saturating_sub(kind + 1 + repo_meta_gap + ci + ci_kind_gap)
            .max(1);
...
fn build_target_map(notifications: &[Notification]) -> HashMap<char, Vec<usize>> {
    ...
    for (idx, notification) in notifications.iter().enumerate() {
        let Some(status) = notification.subject.status else {
            continue;
        };
...
mod tests {
    use super::{
        base_notification_style, build_pending_map, layout_widths, truncate_with_suffix,
        READ_NOTIFICATION_COLOR,
    };
    use crate::types::Action;
    use ratatui::style::Style;
    use crate::types::{Notification, Repository, Subject, SubjectStatus};
    ...
    fn layout_widths_fit_in_area() {
        let widths = layout_widths(40, 12, 26, 11, 3, 1);
